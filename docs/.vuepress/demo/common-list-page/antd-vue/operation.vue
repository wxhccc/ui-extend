<template>
  <div :style="{ height: '400px' }">
    <ue-common-list-page v-model:selection-value="selectionValue" v-model:search-params="searchParams"
      :loading="loading" :columns="columns" :paged-data="pagedData" :operation="operation"
      :get-paged-data="getPagedData" created-auto-send>
    </ue-common-list-page>
  </div>
</template>
...
import { SearchInput, vwp } from '@wxhccc/ue-antd-vue'
...
export default defineComponent({
  setup() {
    ...
    const deleteChoose = () => {
      ...
      Modal.confirm({
        title: '提示', content: `你确定要删除id为${selectionValue.value.join(',')}的记录么？`, onOk: () => {
