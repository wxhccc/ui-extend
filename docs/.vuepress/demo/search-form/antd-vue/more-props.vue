<template>
  <div>
    <p>{{ formData }}</p>
    <ue-search-form :label-col="{ style: { width: '80px' } }" :items="items" :button-options="buttonOptions" v-model="formData">
    </ue-search-form>
  </div>
</template>
...
export default defineComponent({
  setup() {
    const formData = ref({})
    const buttonOptions = {
      searchBtnProps: { shape: 'round' },
      btnText: '搜索'
    }
    const items = [
      createInputFormItem('活动名称', 'name'),
