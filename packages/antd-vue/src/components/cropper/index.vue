...
const getCroppedFile = () => new Promise((resolve) => callCropperMethod('getCroppedCanvas').toBlob(resolve))

const callCropperMethod = <K extends keyof Cropper>(fn: K, ...args: Parameters<Cropper[K]>) => {
  if (!$cropper.value) {
    return
  }
  const method = $cropper.value[fn]
  if (isFunction(method)) {
    return method.call($cropper.value, ...args)
  }
}

defineExpose({ resetCropper, getCroppedFile, callCropperMethod })
