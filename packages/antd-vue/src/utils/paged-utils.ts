...
/* 分页接口数据格式 */
export interface PagedData<R extends AnyObject> {
  rows?: R[]
  /** 总记录条数，用于页码分页 */
  total?: number
  /** 是否有下一页，用于下拉分页 */
...
>(props: CP, context: CTX, storeSession: ReturnType<typeof useStorage>) {
  ...
  const pagination = computed<false | UePaginationProps>(() => {
    ...
    const typeProps = isTotalMode.value
    ? {
        showTotal: (total: number) => `共${Math.ceil(total / pageSize)}页 / 共${total}条数据`,
        total: total.value
      }
    : {
        class: 'no-total-pagination',
        simple: true,
    ...
    return {
      ...
      ...(pagination === true ? {} as UePaginationProps : pagination),
      current: curPage || 1,
      pageSize,
      ...typeProps,
      'onUpdate:current': (val) => onPagedChange({ curPage: val }),
      'onUpdate:pageSize': (val) => onPagedChange({ pageSize: val })
  ...
  return { isTotalMode, pagination, dataList, storeSession, getStoreData, clearStoreData }
}
