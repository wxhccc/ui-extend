          ...
          <template v-else-if="!getItemEditFlag(item)">
            ...
            <template v-else>
              <span v-if="valueNoWrap" class="info-text ue-text-ellipsis">
                <ue-tooltip :content="getItemValue(item, secretFlags[getItemKey(item)])">
...
.ue-info-table {
  align-items: stretch;
  margin-left: 1px;
  --ue-text-color: var(--el-text-color-primary, #000000);
  --ue-label-text-color: var(--el-text-color-secondary, rgba(#000000, 0.45));
  --ue-border-color: var(--el-border-color, #ebeef5);
