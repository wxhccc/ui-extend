...
import { UeScrollbar } from '@/ui-comps'
import { vueTypeProp } from '@/utils/component'
import { LoadingOutlined } from '@/ui-comps/icons'
...
const bottomScope = computed(() => {
  return { hasScroll: hasScroll.value, isEmpty: isEmpty.value, isNoMore: isNoMore.value }
})

const handleScroll = computed(() => debounce(checkReachBottom, props.debounceWait))

const checkCanScroll = () => {
  const { clientHeight = 0, scrollHeight = 0 } = scrollWrap.value || {}
