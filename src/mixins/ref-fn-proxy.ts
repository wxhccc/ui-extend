...
export default function (refKey: string, methodNames: string[] = [], includePrivate = false) {
  ...
  methodNames.forEach(name => {
    ...
    methods[name] = function(this: ComponentPublicInstance, ...args) {
      ...
      refComps && refComps[name as keyof typeof refComps] === 'function' && refComps[name as keyof typeof refComps](...args)
    }
  })
  return { methods };
}

export function pluginMethodsProxyMixin (instanceKey: string, methodNames: string[] = []) {
  const methods: Record<string, () => unknown> = {};
  methodNames.forEach(name => {
