import { dateFormat, commaNum } from 'UE/utils'
...
// 时间格式化函数
export const dateFormatter = (format = '') => (row, column, cellValue, index) => dateFormat(cellValue, format)
...
export function createInputFormItem (label, prop, placeholder, extraProps = {}, slots) {
  const slotComp = typeof slots === 'string' ? { slots: { 'append': { component: 'div', text: slots } } } : slots;
  return createFormFieldItem('ElInput', label, prop, {
...
// 生成下拉表单项配置对象
export function createSelectFormItem (label, prop, data, allDef, placeholder, fieldExtra, extraProps) {
  return createFormFieldItem('ElSelect', label, prop, {
...
export function createDateFormItem (label, prop, type, fieldExtra, extraProps) {
  ...
  return createFormFieldItem('ElDatePicker', label, prop, {
